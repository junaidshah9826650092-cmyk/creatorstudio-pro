...
/* --- Extracted from index.html --- */
            ...
            function renderVideos(videosToRender, isSimulatedDrama = false) {
                ...
                // Reset Layout for Feed - Handle via CSS classes
                document.body.classList.remove('hide-ui');
                const sidebar = document.querySelector('.sidebar');
                if (sidebar) sidebar.classList.remove('hidden-always');
            ...
            function loadView(viewName, data = {}) {
                ...
                if (catBar) catBar.style.display = 'flex';
                if (topBar) topBar.style.display = 'flex';
                    // Layout handled via CSS classes
                ...
                if (viewName === 'Live Stream') {
                    ...
                    // Full screen dashboard mode (Handle via CSS class)
                    document.body.classList.add('hide-ui');
                    ...
                    <!-- Inline CSS extracted to style.css -->
                    `;
            ...
            // --- Dynamic Site Configuration (Admin Power) ---
            async function loadAdminConfig() {
                try {
                    const res = await fetch('/api/admin/settings');
                    const config = await res.json();
                    
                    // Apply Site Name & Tagline
                    if (config.site_name) {
                        document.title = `${config.site_name} | The Next-Gen Video Platform`;
                        document.querySelectorAll('.logo-text').forEach(el => el.innerText = config.site_name);
                    }
                    
                    // Apply Custom CSS
                    if (config.custom_css) {
                        document.getElementById('admin-custom-css').innerHTML = config.custom_css;
                    }
                    
                    // Apply Announcement Banner
                    if (config.announcement_banner && config.announcement_banner.trim() !== '') {
                        const banner = document.getElementById('admin-announcement-banner');
                        document.getElementById('announcement-text').innerHTML = config.announcement_banner;
                        banner.style.display = 'block';
                    }
                    
                    // Apply Spotlight Title & Desc
                    if (config.spotlight_title) {
                        const sTitle = document.getElementById('spotlight-title');
                        if (sTitle) sTitle.innerText = config.spotlight_title;
                    }
                    if (config.spotlight_desc) {
                        const sDesc = document.getElementById('spotlight-desc');
                        if (sDesc) sDesc.innerText = config.spotlight_desc;
                    }
                            ...
                            <div style="display: flex; justify-content: space-between; align-items: center; flex-wrap: wrap; gap: 15px;">
                                ...
                                <div style="background: rgba(255,255,255,0.04); border: 1px solid rgba(255,255,255,0.05); border-radius: 16px; padding: 16px; margin-top: 24px; font-size: 0.95rem;">
                                    <div style="font-weight: 800; margin-bottom: 10px; color: #fff; display: flex; justify-content: space-between; align-items: center;">
                                        <span><span id="view-count">${initialViews}</span> views • Published on ${timeAgo}</span>
                                        <div style="display: flex; gap: 8px;">
            ...
            function showStudioAlert(type) {
                ...
                if (type === 'sub') {
                    alertEl.innerHTML = `<i class="fas fa-star" style="color:#ff0055;font-size:1.4rem;"></i> <span>${user} joined the Squad! ⚡</span>`;
                    updateSubGoal();
                } else {
                    const amounts = ["₹100", "₹500", "₹1000", "₹50", "₹200"];
                    const amount = amounts[Math.floor(Math.random() * amounts.length)];
                    alertEl.style.borderColor = "#ffd700";
                    alertEl.style.boxShadow = "0 0 30px rgba(255,215,0,0.4)";
                    alertEl.innerHTML = `<i class="fas fa-gift" style="color:#ffd700;font-size:1.4rem;"></i> <span>${user} sent a gift of ${amount}! 🎁</span>`;
                }
                ...
                try {
                    const res = await fetch('/api/video/upload', {
                        ...
                        body: JSON.stringify({
                            email: user.email,
                            title: title,
                            description: "🔴 Live Stream via Vitox Studio",
            ...
            async function openCollabChat(creatorEmail, force=false) {
                ...
                try {
                    ...
                    if (targetEl) targetEl.innerHTML = '@' + creatorEmail.split('@')[0] +
                        (data.type === 'brand' ? ' <span style="background:#ffd700;color:#000;font-size:0.6rem;padding:2px 6px;border-radius:4px;font-weight:800;vertical-align:middle;">??? BRAND</span>' : '');
                ...
                if (reason === 'login') {
                    ...
                    content = `
                        ...
                        <div style="display:flex;gap:10px;width:100%;">
                            ...
                            <button onclick="document.getElementById('collab-locked-modal').remove(); showBrandRegistration();" style="flex:2;background:linear-gradient(135deg,#ffd700,#ff5500);color:#000;border:none;padding:12px;border-radius:12px;cursor:pointer;font-weight:800;">??? Register as Brand</button>
                        </div>
                    `;
            ...
            async function toggleLike(videoId, channelEmail) {
                ...
                try {
                    ...
                    const data = await res.json();
                    showToast(data.status === 'liked' ? "Liked! ❤️" : "Like Removed");
            ...
            function showDailyRewardModal() {
                ...
                document.getElementById('claim-reward-btn').onclick = () => {
                    ...
                    showToast("🎁 50 Points Added! 2x Progress Active.");
            ...
            function showPricing() {
                ...
                modal.innerHTML = `
                    <div class="collab-modal" style="max-width: 500px; padding: 40px; text-align: center; border: 2px solid #ffd700; background: #0a0a0a; border-radius: 32px; box-shadow: 0 0 50px rgba(255,215,0,0.15);">
                        ...
                        <div style="font-size: 2rem; font-weight: 800; color: white; margin-bottom: 24px;">₹99<span style="font-size: 1rem; color: #666; font-weight: 400;">/month</span></div>
            ...
            function startRazorpayPayment() {
                ...
                const options = {
                    ...
                    "handler": function (response){
                        // Payment Success
                        showToast(`🚀 Success! Payment ID: ${response.razorpay_payment_id}`);
            ...
            async function upgradeUserToPremium(paymentId) {
                ...
                try {
                    ...
                    const data = await res.json();
                    if (data.status === 'success') {
                        showToast("👑 Welcome to the Squad! You are now Premium.");
                    ...
                    if (preview) {
                        preview.innerHTML = `
                            ...
                            <div style="position: absolute; bottom: 0; left: 0; right: 0; background: rgba(0,0,0,0.85); padding: 20px; font-size: 0.85rem; color: #eee; backdrop-filter: blur(8px); transform: translateY(100%); transition: 0.3s;" id="ai-asset-details" onmouseenter="this.style.transform='translateY(0)'" onmouseleave="this.style.transform='translateY(calc(100% - 10px))'">
                                ...
                                <div style="color: #ff0055; font-weight: 800; margin-bottom: 8px; font-size: 0.75rem; letter-spacing: 1px;">?? ENHANCED PROMPT</div>
            ...
            function incrementWatchCount() {
                ...
                // Dopamine hits
                if (count === 1) { showToast("🎉 Badge Unlocked: First Blood!"); }
                if (count === 5) { showToast("🏆 Badge Unlocked: Avid Watcher!"); }
                if (count === 10) { showToast("👑 Badge Unlocked: Vitox Master!"); }
...
/* --- Extracted from index.html --- */
    ...
    (function() {
        // Fetch today's festival from server (AI-managed dates)
        fetch('/api/festival')
            .then(r => r.json())
            .then(data => {
                if (data && data.active) launchFestivalTheme(data);
            })
            .catch(() => {
        ...
        function localFestivalCheck() {
            const today = new Date();
            const m = today.getMonth() + 1, d = today.getDate();
            // Quick local fallback list
            const quick = [
                { m:3, d:3, active:true, name:'Holi', msg:'?? Happy Holi from Vitox! Rang Barse! ??', effect:'holi', bg:'linear-gradient(90deg,#ff0055,#ff6600,#ffcc00,#00cc66,#0066ff,#cc00ff)', color:'#fff' }
            ];
            const f = quick.find(x => x.m === m && x.d === d);
            if (f) launchFestivalTheme(f);
        }

        function launchFestivalTheme(f) {
            const banner = document.getElementById('festival-banner');
            if (banner) {
                banner.style.background = f.bg;
                banner.style.color = f.color || '#fff';
                banner.style.display = 'block';
                banner.style.textShadow = '0 1px 3px rgba(0,0,0,0.4)';
                document.getElementById('festival-banner-text').textContent = f.msg;
            }
            ...
            let particles = [];
            const effect = f.effect || 'sparkle';

            if (effect === 'holi') {
                const colors = ['#ff0055','#ff6600','#ffcc00','#00cc66','#0099ff','#cc00ff','#ff99cc','#00ffcc'];
                for (let i = 0; i < 150; i++) {
                    particles.push({ x: Math.random()*canvas.width, y: -Math.random()*canvas.height, r: Math.random()*14+4, color: colors[Math.floor(Math.random()*colors.length)], speed: Math.random()*2+0.5, wobble: Math.random()*3-1.5, alpha: Math.random()*0.7+0.3, rotate: Math.random()*360, rotateSpeed: Math.random()*4-2 });
                }
            } else if (effect === 'sparkle') {
                for (let i = 0; i < 80; i++) {
                    particles.push({ x: Math.random()*canvas.width, y: Math.random()*canvas.height, r: Math.random()*3+1, color: ['#ffd700','#fff','#ff6600','#ffcc00'][Math.floor(Math.random()*4)], speed: Math.random()*1+0.2, wobble: Math.random()*2-1, alpha: Math.random(), fade: Math.random()*0.02+0.005 });
                }
            } else if (effect === 'tricolor') {
                const colors = ['#ff6600','#ffffff','#128807'];
                for (let i = 0; i < 90; i++) {
                    particles.push({ x: Math.random()*canvas.width, y: -Math.random()*canvas.height, r: Math.random()*8+3, color: colors[Math.floor(Math.random()*3)], speed: Math.random()*2+0.5, wobble: Math.random()*2-1, alpha: 0.8 });
                }
            } else if (effect === 'snow') {
                for (let i = 0; i < 100; i++) {
                    particles.push({ x: Math.random()*canvas.width, y: -Math.random()*canvas.height, r: Math.random()*6+2, color: '#fff', speed: Math.random()*1.5+0.3, wobble: Math.random()*1.5-0.75, alpha: Math.random()*0.7+0.3 });
                }
            }
            ...
            const MAX = 600;
            function animate() {
                if (frames++ > MAX) { ctx.clearRect(0,0,canvas.width,canvas.height); return; }
                ...
                particles.forEach(p => {
                    ...
                    ctx.restore();
                    if (p.isSplash) {
                        p.vx *= 0.95; p.vy += p.gravity || 0.3;
                        p.x += p.vx; p.y += p.vy;
                        p.alpha -= p.decay || 0.02;
                        if (p.alpha <= 0) { particles.splice(particles.indexOf(p), 1); }
                    } else { p.y += p.speed; p.x += p.wobble; }
                    if (effect === 'sparkle') {
                        p.alpha -= p.fade||0.01;
                        if (p.alpha <= 0) { p.alpha=1; p.x=Math.random()*canvas.width; p.y=Math.random()*canvas.height; }
                    }
